...
var Utils = (function(my) {
    ...
    my.Vector3.prototype = {
        ...
        normalize: function() {
            var len = this.length();
            // 2.4: Guard against (near) zero-length normals - e.g. a perfectly
            // flat mesh produces zero-vector cross products. Use an epsilon so
            // we never divide by a vanishingly small length and emit NaNs.
            if (len > 1e-9) {
                var inv = 1 / len;
    			this.x *= inv;
    			this.y *= inv;
    ...
    // Build a Mesh (vertices + area-weighted vertex normals + faces) from a flat
    // position buffer and a list of polygon faces (triangles and/or quads).
    //
    // Unlike createMesh() - which assumes a regular width x length quad grid -
    // this accepts arbitrary topology, so it serves the non-rectangular grid
    // pattern generators (hexagonal, triangular, circular). The faces array
    // holds index lists into `positions` (3 indices for a tri, 4 for a quad).
    my.buildMesh = function(positions, faces) {

        var mesh = new my.Mesh(0, 0);

        var vCount = (positions.length / 3) | 0;
        for (var i = 0; i < vCount; ++i) {
            var b = i * 3;
            mesh.vertices.push( new my.Vector3(positions[b], positions[b + 1], positions[b + 2]) );
            mesh.vertNormals.push( new my.Vector3() );
        }

        // Accumulate a face normal onto each of the face's vertices using
        // Newell's method. Newell yields a correct normal for any (convex)
        // polygon, and its magnitude is proportional to the face area - so the
        // accumulation is area-weighted for free (same intent as createMesh).
        for (var f = 0; f < faces.length; ++f) {
            ...
            var nx = 0, ny = 0, nz = 0;
            for (var k = 0; k < n; ++k) {
                var vc = mesh.vertices[idx[k]];
                var vn = mesh.vertices[idx[(k + 1) % n]];
                nx += (vc.y - vn.y) * (vc.z + vn.z);
                ny += (vc.z - vn.z) * (vc.x + vn.x);
                nz += (vc.x - vn.x) * (vc.y + vn.y);
            }

            var faceNormal = new my.Vector3(nx, ny, nz);
            for (var k2 = 0; k2 < n; ++k2) {
                mesh.vertNormals[idx[k2]].add(faceNormal);
            }

            mesh.faces.push(idx);
        }

        // Normalize (createMesh's 1e-9 epsilon guard lives in Vector3.normalize).
        for (var iv2 = 0; iv2 < vCount; ++iv2) {
            mesh.vertNormals[iv2].normalize();
        }
    ...
    my.createOBJString = function(name, mesh) {
        ...
        // 2.1: Fusion imports this OBJ with MillimeterMeshUnit (see Image2Surface.py)
        // and the vertex coordinates are authored in mm, so declare mm here too.
        objStr += "# Units = Millimeters \n";
        objStr += "# Face count = " + mesh.faces.length + " \n";
        objStr += "# Vertext count = " + mesh.vertices.length + " \n\n";

        if (mesh && mesh.vertices && mesh.vertices.length >= 3 && mesh.faces.length > 0) {
    ...
    my.createOBJSimple = function(heightData, width, length, step) {
        ...
        for (var y = 0; y < length - 2; ++y) {
             for (var x = 0; x < width - 2; ++x) {
                ...
                objStr += stringifyVertexOBJ( p1 );
                objStr += stringifyVertexOBJ( p2 );
                objStr += stringifyVertexOBJ( p3 );
                objStr += stringifyVertexOBJ( p4 );
